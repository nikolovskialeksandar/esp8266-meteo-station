...
String FIREBASE_HOST = FIREBASE_PROJECT_ID;
String FIREBASE_AUTH = FIREBASE_DATABASE_SECRET;
...
void loop() {
  ...
  String year = ntp.formattedTime("%Y");
  String month = ntp.formattedTime("%m");
  String day = ntp.formattedTime("%e");
  ...
  String url = "/meteoData/" + year + "/" + month + "/" + day;
  Serial.println(day);
  ...
  if (Firebase.pushJSON(firebaseData, url, jsonDataLoad)) {
